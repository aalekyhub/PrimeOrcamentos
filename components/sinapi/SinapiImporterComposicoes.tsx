...
const SinapiImporterComposicoes: React.FC = () => {
    ...
    const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            const records = await sinapiParsers.parseComposicoes({ file, ...config });
            ...
            <label className="block border-2 border-dashed border-slate-100 rounded-xl p-6 text-center hover:bg-slate-50 transition-all cursor-pointer">
                {isProcessing ? (
                    ...
                    <div className="flex flex-col items-center">
                        ...
                        <span className="text-[10px] font-bold text-slate-500 uppercase">Clique para subir XLSX ou CSV de Composições</span>
                    </div>
                )}
...
export default SinapiImporterComposicoes;
