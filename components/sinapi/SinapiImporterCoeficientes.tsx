...
const SinapiImporterCoeficientes: React.FC = () => {
    ...
    const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            const records = await sinapiParsers.parseCoeficientes({ file, ...config });
            ...
            notify(`Sucesso: ${records.length} coeficientes importados para ${config.uf} (${config.mes_ref})`);
        } catch (err: any) {
            notify(err.message || 'Erro ao importar coeficientes', 'error');
        } finally {
    ...
    return (
        <div className="bg-white p-6 rounded-2xl border border-slate-200 space-y-4 shadow-sm">
            <div className="flex items-center gap-3 mb-2">
                ...
                <h4 className="font-black text-slate-900 uppercase tracking-tighter">Importar Famílias (Coeficientes/BOM)</h4>
            </div>
            ...
            <label className="block border-2 border-dashed border-slate-100 rounded-xl p-6 text-center hover:bg-slate-50 transition-all cursor-pointer">
                {isProcessing ? (
                    ...
                    <div className="flex flex-col items-center">
                        ...
                        <span className="text-[10px] font-bold text-slate-500 uppercase">Clique para subir XLSX ou CSV de Coeficientes</span>
                    </div>
                )}
...
export default SinapiImporterCoeficientes;
