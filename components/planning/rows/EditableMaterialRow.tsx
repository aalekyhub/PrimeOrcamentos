...
interface Props {
    ...
    onUpdate: (updated: PlannedMaterial) => void;
    onDelete: (id: string) => void;
}

export const EditableMaterialRow: React.FC<Props> = ({
    material,
    index,
    totalItems,
    isDragged,
    ...
    const [editDesc, setEditDesc] = useState(material.material_name);
    const [editUnit, setEditUnit] = useState(material.unit);
    const [editQty, setEditQty] = useState(material.quantity);
    const [editPrice, setEditPrice] = useState(material.unit_cost);
        ...
        >
            <div className="flex items-center gap-2 grow">
                ...
                <div className="grow text-slate-900 dark:text-slate-100 min-w-0">
                    <span className="break-words"><b className="dark:text-blue-400 uppercase">{material.material_name}</b> | (R$ {material.unit_cost.toFixed(2)}) {material.quantity} {material.unit}</span>
                </div>
            </div>
            <div className="flex items-center gap-4 text-xs shrink-0">
                <div className="w-32 text-right">
                    <span className="font-bold text-slate-900 dark:text-slate-100 whitespace-nowrap">R$ {material.total_cost.toFixed(2)}</span>
                </div>
