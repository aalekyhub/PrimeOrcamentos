                ...
                <tr key={entry.id} className="group hover:bg-slate-50/50 dark:hover:bg-slate-700/30 transition-all duration-300">
                  <td className="px-8 py-5">
                    <div className="flex items-center gap-3">
                      {entry.status === 'PAGO' ? (
                        ...
                        <div className="flex items-center gap-2 px-2 py-1 bg-rose-50 text-rose-600 rounded-lg">
                          <AlertCircle className="w-3 h-3" />
                          <span className="text-[10px] font-black uppercase tracking-tighter">Atrasado</span>
                        </div>
                      ) : (
                        <div className="flex items-center gap-2 px-2 py-1 bg-amber-50 text-amber-600 rounded-lg">
                    ...
                    <div className="flex flex-col">
                      <div className="flex items-center gap-2 group/desc">
                        {entry.type === 'RECEBER' || entry.type === 'INVESTIMENTO' ? (
                          <div className="p-1 px-1.5 bg-emerald-50 text-emerald-600 rounded-md">
                            <ArrowUpRight className="w-3 h-3" />
                          </div>
                        ) : (
                          <div className="p-1 px-1.5 bg-rose-50 text-rose-600 rounded-md">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <span className="text-[9px] font-bold text-slate-400 uppercase bg-slate-100 dark:bg-slate-700/50 px-1.5 py-0.5 rounded">
                          {entry.category}
                        </span>
                        {(entry.customerName || entry.supplierName) && (
                          <span className="text-[9px] font-bold text-blue-500 uppercase bg-blue-50 dark:bg-blue-900/30 px-1.5 py-0.5 rounded">
                            {entry.customerName || entry.supplierName}
                          </span>
                        )}
                      </div>
                  ...
                  <td className="px-6 py-5">
                    <div className="flex flex-col">
                      ...
                      {entry.paymentDate && (
                        <span className="text-[9px] font-medium text-emerald-500 mt-0.5 italic">
